    ...
    public partial class ProfileViewModel : ViewModelBase
    {
        ...
        private async Task LoadProfileAsync()
        {
            ...
            if (barber != null)
            {
                ...
                BarberPhone = barber.PhoneNum;
                BarberDescription = barber.Description ?? "";
                if (!string.IsNullOrEmpty(barber.ProfileImage))
                {
                    try
                    {
                        string cleanBase64 = barber.ProfileImage.Contains(",") ? barber.ProfileImage.Split(',')[1] : barber.ProfileImage;
        ...
        public async Task ProcessImage(string filePath)
        {
            try
            {
                ...
                _imageBase64 = "data:image/png;base64," + Convert.ToBase64String(imageBytes);
                StatusMessage = "Kép kiválasztva!";
                StatusMessageColor = "Blue";
            }
            catch
            {
                StatusMessage = "Hiba a képnél.";
                StatusMessageColor = "Red";
            }
        }
        ...
        {
            string phoneToSave = BarberPhone.Trim();
            if (!string.IsNullOrEmpty(phoneToSave) && !phoneToSave.StartsWith("+")) phoneToSave = "+" + phoneToSave;

            if (string.IsNullOrWhiteSpace(BarberName) || BarberName.Length < 2)
            {
                StatusMessage = "Név túl rövid!";
                StatusMessageColor = "Red";
                return;
            }
            if (string.IsNullOrWhiteSpace(phoneToSave) || !phoneToSave.StartsWith("+") || phoneToSave.Length < 10)
            {
                StatusMessage = "Telefon: +36...";
                StatusMessageColor = "Red";
                return;
            }
            ...
            if (!string.IsNullOrEmpty(NewPassword))
            {
                ...
                {
                    StatusMessage = "Jelszó min. 6 karakter!";
                    StatusMessageColor = "Red";
                    return;
                }
            ...
            IsSaving = true;
            StatusMessage = "Mentés...";
            var res = await _api.UpdateBarberProfileAsync(_barberId, BarberName, phoneToSave, string.IsNullOrEmpty(NewPassword) ? null : NewPassword, _imageBase64, BarberDescription);
            StatusMessage = res.Message;
            StatusMessageColor = res.IsSuccess ? "Green" : "Red";
            if (res.IsSuccess) _mainVm.RefreshBarberName(BarberName);
            IsSaving = false;
        }
